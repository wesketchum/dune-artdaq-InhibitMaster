...
def CreateStatusMsg(process,marker,status):
    if status!=STATUS_GOOD and status!=STATUS_BAD:
        print "Status provided (%s) is neither %s or %s. Calling it %s." % (status,STATUS_GOOD,STATUS_BAD,STATUS_BAD)
        status=STATUS_BAD
...
class InhibitMaster:
    ...
    def __init__(self,update_freq=1.0,verbose=False): #timeout in seconds
        self.frontend_dict = { 'INHIBITMASTER':STATUS_GOOD }
